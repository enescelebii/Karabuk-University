...
#include<stdlib.h>
struct students{
	int no,age;												//2110206007 ENES CELEBI 29.10.23 20:37
...
void findLongestStudent(FILE *dosya) {
    int temp = 0;
    char isim[100];
    dosya = fopen("students.dat", "r");
    ...
    for (int i = 0; i < counter; i++) {
        fread(&ogrencibilgi, sizeof(ogrenci), 1, dosya);
        if (strlen(ogrencibilgi.name) > temp) {
            temp = strlen(ogrencibilgi.name);
            strcpy(isim, ogrencibilgi.name);
        }
    }
    system("cls");
    printf("%s	: uzunluk:%d\n\n", isim, temp);
